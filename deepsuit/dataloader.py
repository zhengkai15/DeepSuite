...
def get_dataloader(config, mydataset=None):
    ...
    train_loader = DataLoader(my_data_train, batch_size=config["train"]["batch_size"], num_workers=config["train"]["num_workers"], shuffle=True, worker_init_fn=worker_init_fn)

    my_data_train_eval = mydataset(flag='train',config=config) 
    train_loader_eval = DataLoader(my_data_train_eval, batch_size=1, num_workers=1, shuffle=True,worker_init_fn=worker_init_fn)

    my_data_valid = mydataset(flag='valid',config=config)
    valid_loader = DataLoader(my_data_valid, batch_size=1, num_workers=1, shuffle=False,worker_init_fn=worker_init_fn)

...
class PhyDNet_Model(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        C, H, W = config["in_varibale_num"], config["h"], config["w"]   
        self.C_out = config["out_varibale_num"]   
    
        self.pre_seq_length, self.aft_seq_length = config["in_times_num"], config["out_times_num"]
...
if __name__ == "__main__":
    ...
    frames_gt = torch.randn(2, 1, 1, 128, 128).to('cuda')

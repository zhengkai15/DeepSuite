...
class SmaAt_UNet(nn.Module):
    def __init__(self, params, kernels_per_layer=2, bilinear=True, reduction_ratio=16):
        super(SmaAt_UNet, self).__init__()
        self.n_channels = params['in_channels']
        self.n_classes = params['out_channels']
    ...
    def forward(self, x):
        ...
        # base_line = base_line.repeat(1, int(self.n_classes/C), C, 1, 1)  # 初始场作为训练时候的baseline
        x = x.reshape(B, -1, W, H)
        x1 = self.inc(x)
        x1Att = self.cbam1(x1)
        x2 = self.down1(x1)
        ...
        x = self.up4(x, x1Att)
        logits = self.outc(x)
        logits = logits.reshape(B, int(self.n_classes/C), C, W, H)
        return base_line, logits
...
if __name__ == "__main__":
    ...
    model = SmaAt_UNet(params=params)
    sample = torch.randn(1, in_times, in_varibales, 181, 361)
    print(sample.shape)
    print(model(sample)[0].shape, model(sample)[1].shape)
